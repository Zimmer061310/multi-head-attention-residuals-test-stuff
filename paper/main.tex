...
learned softmax, over the outputs of \emph{all} previous layers, an attention
over depth. But that read uses a \emph{single} query shared across the entire
width, so every feature subspace must read the depth history through one
...
model width. We introduce \textbf{Multi-Head Attention Residuals (MHAR)}:
reshape the routing query into $H$ per-subspace heads, each with its own softmax
...
residuals exactly. Trained from scratch on FineWeb-Edu, MHAR improves validation
loss over a standard Transformer at 100M, 350M, and 1B ($-0.049$, $-0.080$,
$-0.063$). What grows with scale is the \emph{necessity} of multiple heads: a
single routing query helps at 100M but by 1B is $0.105$ \emph{worse} than the
plain additive residual, even at its own optimal learning rate, so MHAR's
advantage over single-head routing widens from $0.010$ to $0.168$. Setting the
number of routing heads equal to the number of KV heads is a
hyperparameter-free, near-optimal default, and a direct probe of the trained
queries confirms learned subspace disagreement as the driver. Finally, fused
Triton routing kernels lift attention-residual training from $0.2$--$0.5\times$
to $0.55$--$0.88\times$ of baseline throughput at near-baseline peak memory.
An identity-preserving conversion via \emph{delta} attention residuals
supports 8B mid-training ($+3.2$ GSM8K, $+3.1$ GPQA).
\end{abstract}
...
how collinear (hard to tell apart) the candidate sources are. Single-query
routing should therefore help small models and hurt large ones; splitting the query
into per-subspace heads makes the read block-diagonal and removes the compromise. We confirm the width prediction both in
...
attention residuals exactly at $H{=}1$, and needs no tuning---setting $H$ equal
to the number of KV heads~\citep{ainslie2023gqa} is a near-optimal,
hyperparameter-free default (Appendix~\ref{app:kvh100}).
...
\item \textbf{Scaling MHAR.} Trained from scratch at 100M / 350M / 1B
(\S\ref{sec:exp}), MHAR improves validation loss over a standard Transformer at
every scale ($-0.049$/$-0.080$/$-0.063$ at 100M/350M/1B), while the published
single-head routing degrades from helpful to \emph{harmful}
($-0.039\rightarrow+0.055\rightarrow+0.140$;
\S\ref{sec:ablation}, Appendix~\ref{sec:mechanism}). An identity-preserving
conversion via \emph{delta} attention residuals~\citep{luo2026delta} extends
MHAR to 8B mid-training on a public quality-filtered anneal dataset, adding
$+3.2$ GSM8K and $+3.1$ GPQA (paired McNemar $p{=}0.004/0.038$) over a
schedule-matched plain-CPT control (\S\ref{sec:midtrain}).
\end{itemize}

Appendix~\ref{sec:related} situates MHAR among residual, dense-connection, and
cross-layer-aggregation approaches to reading the depth history.
...
FineWeb-Edu~\citep{penedo2024fineweb} for 20K steps with the AdamW
optimizer~\citep{loshchilov2019adamw}, a cosine learning-rate schedule with
1{,}000 warmup steps decaying to $0.1\times$ peak, and bfloat16. The three scales
...
KV heads. All four share architecture, data, schedule, optimizer, and data-parallel degree,
so the routing mechanism is the only difference. Table~\ref{tab:main} reports each
method at the tuned peak learning rate $1\times10^{-3}$ (the optimum for baseline,
hyper-connections, and MHAR); the full per-method sweep is in
Table~\ref{tab:lrsweep}. MHAR is parameter- and FLOP-matched to baseline up to the
$O(d)$ routing queries. Full training and reproducibility details (optimizer
...
averaged over eleven evals. We additionally train three seeds per method at every
scale and report the \emph{paired} seed deltas (Table~\ref{tab:seeds}); the
single-cell tables report a single fixed seed (seed 42; Appendix~\ref{app:repro}).

\subsection{Pretraining}

\begin{table}[h]
\centering
\caption{From-scratch validation loss ($\downarrow$, tail-mean of the last eleven
evaluations, steps 15--20k; \S\ref{sec:exp}) at the tuned peak learning rate
$1\times10^{-3}$; $\Delta$ is relative to the same-rate baseline;
CEG$_{\mathrm{FLOPs}}$ is the compute-equivalent gain of MHAR over the baseline
(see text). MHAR is best at every scale; single-head routing regresses below
baseline at 350M and 1B.}
\label{tab:main}
...
baseline at all three scales but \emph{inconsistently}: the gain peaks at 350M
($-0.030$) and collapses to within eval noise at 1B ($-0.002$; Table~\ref{tab:main}).
\textbf{MHAR outperforms hyper-connections at every scale} (by $0.035$, $0.050$, and
$0.061$ at 100M/350M/1B), and the margin \emph{widens} with scale: enriching the
depth \emph{routing} of a single stream scales better than enriching the
\emph{connection topology} across streams.

\paragraph{Compute-equivalent gain.} To express the loss deltas as compute,
Table~\ref{tab:main} also reports the compute-equivalent
gain~\citep{davidson2023ceg,mai2026thinking}: the factor by which baseline
training FLOPs would have to grow to match MHAR's loss, from a power-law fit
$L{=}AC^{-\alpha}{+}E$ to the three baseline points, normalized so the
baseline's own CEG is $1$ at each scale. Because MHAR's routing adds only
$0.5$--$1.2\%$ FLOPs, the loss gain translates into a $1.3$--$1.5\times$
compute equivalence under our fixed-step recipe. The comparison is corroborated
by per-method LR tuning (Table~\ref{tab:lrsweep}) and three-seed error bars at
$5\times10^{-4}$ (Table~\ref{tab:seeds}); one caveat: the fixed-step ladder's
flat exponent ($\alpha{\approx}0.06$) inflates CEG relative to compute-optimal
ladders, where the same deltas would yield ${\sim}1.1$--$1.15\times$.
...
100M shown). Since the two runs share
node, software, data order, and global batch, the separation is attributable to
the routing mechanism alone.

\paragraph{Why the advantage grows with scale.} A single routing query shares
one depth distribution across all $d$ channels---a forced compromise that
Table~\ref{tab:main} shows turning from harmless (100M) into a liability (1B)
as width raises how much independent subspaces disagree about what to read
(probed in Appendix~\ref{sec:mechanism}). Figure~\ref{fig:perhead} shows the
trained MHAR heads use their per-slice freedom: each maintains a stable,
distinctive deviation from the head-consensus routing. The advantage survives
scale because the $H$ heads carry $H$ different links, not $H$ copies of one.
...
\paragraph{Robustness.} The ranking in Table~\ref{tab:main} is stable across peak learning rate (each method at its own best), training budget, and random seed (Appendix~\ref{app:robustness}).

\paragraph{Downstream evaluation.} To check that the loss improvement is not an
artifact of the training distribution, we evaluate the baseline and MHAR checkpoints
...
the LM evaluation harness~\citep{eval-harness} (Table~\ref{tab:downstream}). At
\emph{every} scale MHAR improves perplexity and LAMBADA accuracy, with HellaSwag
better at 350M, tied at 1B, and within noise at 100M. The val-loss gain therefore
transfers to held-out data, and the relative perplexity gain \emph{grows} with
scale ($-10\%$, $-15\%$, $-19\%$ at 100M/350M/1B); broader benchmarks are future
work.

\begin{table}[h]
\centering
\caption{Zero-shot downstream evaluation at 100M, 350M, and 1B (single run). Each model
is evaluated at its training context (100M: seq.\ 2048; 350M/1B: seq.\ 1024), so
perplexity is comparable only \emph{within} a scale. MHAR transfers its gain to held-out
perplexity and LAMBADA at every scale, and HellaSwag at 350M (tied at 1B, marginally lower at 100M). 100M
is the earlier $6\times10^{-4}$ batch; 350M and 1B are the $5\times10^{-4}$ batch.}
\label{tab:downstream}
\begin{tabular}{llccc}
\toprule
\textbf{Scale} & \textbf{Method} & \textbf{WikiText-2 PPL $\downarrow$} & \textbf{LAMBADA $\uparrow$} & \textbf{HellaSwag $\uparrow$} \\
\midrule
100M & Baseline  & 48.6 & 10.4\% & \textbf{33.0\%} \\
100M & MHAR  & \textbf{43.8} & \textbf{13.4\%} & 32.5\% \\
\midrule
350M & Baseline  & 53.9 & 9.6\% & 33.5\% \\
350M & MHAR  & \textbf{46.0} & \textbf{13.4\%} & \textbf{35.5\%} \\
\midrule
1B & Baseline  & 49.6 & 12.4\% & 34.0\% \\
1B & MHAR  & \textbf{40.1} & \textbf{15.0\%} & 34.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
corpus of $\approx 1.9$\,T tokens, from public quality-filtered corpora,
...
checkpoint without a loss spike. Figure~\ref{fig:midtrain_identity} shows this
directly on the schedule-matched pair (identical seed and data order): the loss
curves start numerically identical (first logged step $1.8616$ vs $1.8615$;
maximum gap $6\times10^{-5}$ over the first 100 steps), and the gap saturates
near $10^{-3}$---$36\times$ below step-to-step batch noise---across all 9{,}500
steps.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/midtrain_identity.pdf}
\caption{\textbf{The conversion is exactly identity and introduces no
optimization shock.} Train loss of the schedule-matched 8B mid-training pair
(identical seed and data order). \textbf{Left:} the two curves coincide.
\textbf{Right:} the per-step $|\Delta \mathrm{loss}|$ starts at ${\sim}10^{-5}$,
rises only as the zero-initialized gate opens, and saturates $36\times$ below
step-to-step batch noise (dotted).}
\label{fig:midtrain_identity}
\end{figure}
...
isolates what routing itself adds: a real, reasoning-sided gain of $+3.2$ GSM8K
...
unchanged. The routing gain at this scale is thus modest but consistently
reasoning-sided; how it interacts with a more aggressive re-training schedule
...
1B; Table~\ref{tab:maincost})---far too few to account for the $-0.05$ to
$-0.08$ nat gain. Crucially, the
...
empirically \emph{necessary} as models grow, and that one hyperparameter-free
rule, $H{=}$ KV (the stream's consumption granularity), captures essentially all
of the benefit.
...
router is. In a deliberately \emph{under-trained} grid (100M at its optimal LR
...
Trained to convergence, the optimum instead \emph{saturates} near $H{=}$ KV, the
stream's consumption granularity: at KV$\,{=}\,1$ the converged optimum is the
single matched head $H{=}1$, and the full $4\times4$ sweep at 100M
(Appendix~\ref{app:kvh100}) puts the KV$\,{=}\,4,8$ optima exactly on the $H{=}$ KV
diagonal. We therefore adopt $H{=}$ KV throughout the main results
(Table~\ref{tab:main}).
...
free averaging flexibility; after it, $H{=}$ KV is the knee. Practically, $H{=}$ KV
is the safe default for a \emph{well-tuned} run; under-tuning only ever pushes the
optimum to \emph{more} heads, never fewer, so the rule never costs you.

\paragraph{Single-head routing degrades at scale.}
...
Read against the baseline with \emph{each method at its own optimal learning rate}
(Table~\ref{tab:lrsweep}), it is \emph{not} robust as the model grows. It helps at
100M ($-0.039$), is roughly neutral at 350M ($+0.038$), and clearly regresses at 1B,
where replacing the additive residual with a single routing query is $0.105$
\emph{worse} than the baseline. MHAR ($H{=}$ KV), in
contrast, improves at every scale, so multi-head's advantage over single-head routing
\emph{widens} monotonically: $-0.010$, $-0.118$, $-0.168$ at 100M/350M/1B
(best-vs-best; larger still at a shared rate)---the central claim of this work.


\begin{figure}[t]
\centering
\includegraphics[width=0.5\textwidth]{figures/heads_undertrained.pdf}
\caption{\textbf{The bypass wants more heads.} Under-trained regime (100M,
$1\times10^{-3}$, 5K steps): routing-head ($H$) $\times$ KV grid of validation
loss (lower/yellow better; gold stars: per-KV optimum; red dashed line: the
$H{=}$ KV diagonal). The boxed KV$\,{=}\,1$ row is the one supra-noise signal
(see text).}
...
empirical rule ($H{=}$ KV) but not a theory for why the optimum sits near there,
...
parameter cost over single-head routing. Trained from scratch from 100M to 1B, MHAR
improves over a standard Transformer at every scale while a single routing query
degrades from helpful to harmful, so the multi-head advantage widens with
scale; the gain transfers to held-out perplexity and LAMBADA accuracy. Setting
the number of routing heads equal to the number of KV heads is a near-optimal,
hyperparameter-free default (aligning routing to attention head groups adds nothing
...
beyond research scale. Understanding \emph{why} the optimum sits near $H{=}$ KV, and
...
grid in Figure~\ref{fig:heads_under}. At 100M the KV$\,{=}\,4$ and KV$\,{=}\,8$
optima sit exactly on the $H{=}$ KV diagonal (gold stars on the red line), while at
small KV$\,{\in}\,\{1,2\}$ more heads help beyond $H{=}$ KV (best cell $H{=}8$). This
grid pins down the two practically relevant conclusions: multi-head routing helps at
every KV setting, and $H{=}$ KV is a near-optimal, hyperparameter-free
default in the grouped-query regime (KV$\,{\geq}\,4$) that modern models use. We ran
the same sweep at 350M and 1B. At 350M the KV$\,{\geq}\,4$ optima again fall on the
$H{=}$ KV diagonal. At 1B, however, the global grid argmin is the \emph{off}-diagonal
cell KV$\,{=}\,2$, $H{=}8$ (tail-mean $3.258$), and the four lowest cells---the
off-diagonal KV$2$/$H8$, the on-diagonal ($H{=}$ KV) KV$8$/$H8$, and the near-diagonal
KV$8$/$H4$ and KV$4$/$H8$---lie within $\sim\!0.02$ of one another, below the
$\sim\!\pm0.07$ single-pass eval noise; these sweeps are also single-seed. We
therefore read $H{=}$ KV at 1B as \emph{within noise of optimal} in the grouped-query
regime rather than the strict grid minimum, and summarize the two sweeps here rather
...
KV$\,{=}\,8$ optima sit on the $H{=}$ KV diagonal. The 100M architecture of
Table~\ref{tab:main}, trained at $5\times10^{-4}$.}
\label{fig:kvh100}
\end{figure}
...
The pattern above has a single explanation. Multi-head routing buys almost nothing
...
head-alignment null of \S\ref{sec:ablation}). Source collinearity is \emph{non-monotonic} across scale---it rises from 100M to
...
mixture to its $Q/K/V$ projections, at parameters and FLOPs matched to MHAR. Table~\ref{tab:headalign} compares the two across scales:
...
\begin{table}[h]
\centering
\caption{Head-alignment ablation (validation loss $\downarrow$, final-step eval):
...
(``124M'') under a unified $5\times10^{-4}$ recipe (20K steps, sequence 2048, global
...
so its $d512$ deltas differ slightly from Table~\ref{tab:main}'s $1\times10^{-3}$
numbers. Within each width
...
\begin{table}[h]
\centering
\caption{Width-control (tail-mean validation loss, steps 15--20k). Holding depth
...
\caption{Training loss at 100M (EMA-smoothed): MHAR stays below the standard
...
$5\,{\times}\,10^{-4}$---still regresses at scale ($+0.038$ at 350M, $+0.105$ at 1B,
...
\begin{table}[h]
\centering
\caption{Best-vs-best learning rate: each method at its \emph{own} optimum over peak
...
every scale} under a single shared $5\times10^{-4}$ recipe. We use one shared rate
here---rather than each method's tuned optimum as in the headline
Table~\ref{tab:main}---so that the three methods differ \emph{only} in seed and
routing mechanism; the tuned-optimum comparison, which is the fairer one for
\emph{absolute} numbers, is Table~\ref{tab:main}, and the two agree
(\S\ref{sec:exp}, ``Robustness to learning rate''). Because seed $s$ fixes the data
...
crosses from a significant \emph{help} at 100M ($-0.043$) to neutral at 350M
($-0.005$) to a significant \emph{regression} at 1B ($+0.093$), and is by far the
...
\begin{table}[h]
\centering
\caption{Seed robustness: paired per-seed delta vs.\ baseline (validation loss
...
\paragraph{Tokenizer and data.} We tokenize FineWeb-Edu~\citep{penedo2024fineweb}
with the Qwen3 byte-level BPE tokenizer (loaded from \texttt{Qwen/Qwen3-0.6B}).
Documents are streamed, joined with the end-of-text token (id $151{,}645$), and
...
(multi-head attention residuals), the scaling and transfer results, the $H{=}$ KV rule,
and the mechanism probe, each matched to the corresponding sections and tables.
