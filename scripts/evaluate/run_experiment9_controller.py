#!/usr/bin/env python3
"""Fail-closed two-GPU controller for frozen Experiment 9."""
...
def phase_conditions(phase: str) -> list[dict]:
    return phase_9a_conditions() if phase == "phase-9a" else phase_9b_conditions()
...
def launch_phase(args, phase: str, status: Path) -> None:
    ...
    for worker in range(2):
        name = f"mhar-exp9-{phase}-gpu{worker}"
        if alive(name):
            continue
        ...
        environment.update({
            ...
            "MHAR_EXP9_ROOT": str(args.output_root),
            "MHAR_EXP9_CHECKPOINT": str(args.checkpoint),
            "MHAR_EXP9_ARTIFACT": str(args.artifact),
        })
        subprocess.run(
            [
                "screen", "-L", "-Logfile", str(logs / f"{phase}-gpu{worker}.log"),
                "-dmS", name,
                str(args.repo / "scripts/evaluate/run_experiment9_worker.sh"),
                phase, str(worker),
            ],
            cwd=args.repo,
            env=environment,
    ...
    while True:
        missing = [
            ...
            if not (
                args.output_root / phase / "conditions" / row["id"] / "result.json"
            ).is_file()
        ]
        active = [alive(f"mhar-exp9-{phase}-gpu{worker}") for worker in range(2)]
        markers = [
...
def run_analysis(args, phase: str) -> Path:
    ...
    command = [
        str(args.python), "-m", "src.experiments.experiment9_head_contribution",
        f"analyze-{phase[-2:]}",
        "--results-root", str(args.output_root),
        "--output-dir", str(output),
        "--wandb-mode", "online",
        "--wandb-group", "mhar-exp9-hq8-head-contribution-seed42-step2000",
        "--wandb-run-name", f"mhar-exp9-{phase}-analysis-seed42",
    ]
    if phase == "phase-9b":
        command.extend([
            "--phase-9a-summary",
            str(args.output_root / "analysis/phase-9a/summary.json"),
        ])
...
def finalize(args, phase_9a: Path, phase_9b: Path | None) -> Path:
    final = args.output_root / "analysis" / "final"
    command = [
        str(args.python), "-m", "src.experiments.experiment9_head_contribution",
        "finalize", "--phase-9a-summary", str(phase_9a), "--output-dir", str(final),
    ]
    if phase_9b is not None:
        command.extend(["--phase-9b-summary", str(phase_9b)])
    subprocess.run(command, cwd=args.repo, check=True)
    subprocess.run(
        [
            str(args.python),
            str(args.repo / "figures/gen_fig_experiment9_head_contribution.py"),
            "--analysis-root", str(args.output_root / "analysis"),
            "--output-dir", str(final),
        ],
        cwd=args.repo,
        check=True,
        env={**os.environ, "MPLBACKEND": "Agg", "MPLCONFIGDIR": "/tmp/mhar-exp9-mpl"},
    )
    for required in ("FINAL_REPORT.md", "final_summary.json"):
        if not (final / required).is_file():
            raise RuntimeError(f"finalization is missing {required}")
    for required in ("fig_head_contribution.pdf", "fig_head_contribution.png"):
        if not (final / required).is_file():
            raise RuntimeError(f"finalization is missing {required}")
    if phase_9b is not None:
        for required in ("fig_local_chunk_alignment.pdf", "fig_local_chunk_alignment.png"):
            if not (final / required).is_file():
                raise RuntimeError(f"finalization is missing {required}")
    return final
...
def backup_push(args, status: Path, final: Path) -> str:
    destination = args.repo / "results/experiment9"
    ...
    shutil.copytree(args.output_root / "phase-9a", destination / "phase-9a")
    if (args.output_root / "phase-9b").exists():
        shutil.copytree(args.output_root / "phase-9b", destination / "phase-9b")
    ...
    for source, target in (
        (args.output_root / "phase-9a", destination / "phase-9a"),
    ...
    subprocess.run(["git", "add", "results/experiment9"], cwd=args.repo, check=True)
    subprocess.run(
        ["git", "commit", "-m", "results: record Experiment 9 frozen ablations"],
        cwd=args.repo,
        check=True,
    )
    ...
    if remote != commit:
        fail(status, "remote Experiment 9 result commit mismatch")
...
def main() -> None:
    ...
    parser.add_argument("--result-branch", default="codex/experiment-9-head-contribution")
    ...
    if (status / "FAILED.json").exists():
        raise RuntimeError("existing Experiment 9 FAILED.json requires review")
    try:
        ...
        launch_phase(args, "phase-9a", status)
        phase_9a = run_analysis(args, "phase-9a")
        phase_9a_payload = read(phase_9a)
        phase_9b = None
        if phase_9a_payload["local_contribution_gate"]["passed"]:
            write(status / "PHASE_9B_AUTHORIZED.json", {"authorized_at_unix": time.time()})
            launch_phase(args, "phase-9b", status)
            phase_9b = run_analysis(args, "phase-9b")
        else:
            write(status / "PHASE_9B_SKIPPED.json", {
                "skipped_at_unix": time.time(),
                "reason": "frozen Experiment 9A local-contribution gate failed",
            })
        final = finalize(args, phase_9a, phase_9b)
