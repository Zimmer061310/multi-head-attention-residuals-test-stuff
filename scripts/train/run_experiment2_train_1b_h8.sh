...
# Stage B matched control: identical to native H16 training except MHAR H=8
# and run/output identity. Protected milestone checkpoints survive rotation.

MHAR_PYTHON_BIN="${MHAR_PYTHON_BIN:-python3}"
MHAR_REPO_DIR="${MHAR_REPO_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
MHAR_OUTPUT_DIR="${MHAR_OUTPUT_DIR:-/root/autodl-tmp/experiment2/checkpoint-1b-h8-fineweb-edu}"
...
exec "$MHAR_PYTHON_BIN" -m torch.distributed.run \
  ...
  --wandb_group mhar-exp2-stage-b-1b-h8-fineweb-edu \
  --wandb_required \
  --run_name mhar-exp2-stage-b-1b-h8-fineweb-edu-seed42 \
  "${MHAR_RESUME_ARGS[@]}"
