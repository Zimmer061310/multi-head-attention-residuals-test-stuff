...
readarray -t MHAR_BRANCH_FIELDS < <(
...
role = sys.argv[2]
if role not in payload["branches"]:
    raise SystemExit(f"unknown branch role: {role}")
row = payload["branches"][role]
print(payload["seed"])
print(payload["endpoint_step"])
print(row["partition_id"] if row["partition_id"] is not None else "")
PY
)
MHAR_SEED="${MHAR_BRANCH_FIELDS[0]}"
MHAR_ENDPOINT="${MHAR_BRANCH_FIELDS[1]}"
MHAR_PARTITION="${MHAR_BRANCH_FIELDS[2]}"
MHAR_OUTPUT_DIR="${MHAR_OUTPUT_ROOT}/seed-${MHAR_SEED}/${MHAR_ROLE}"
...
if [[ -n "$MHAR_PARTITION" ]]; then
  MHAR_PARTITION_ARGS=(--mixed_partition "$MHAR_PARTITION")
fi
...
exec "$MHAR_PYTHON_BIN" -m torch.distributed.run \
  ...
  --save_every 500 \
  --keep_last 2 \
  --keep_steps "$MHAR_ENDPOINT" \
  --eval_every 0 \
  --log_every 10 \
  --branch_from "$MHAR_PARENT_CHECKPOINT" \
  --branch_manifest "$MHAR_BRANCH_MANIFEST" \
  --branch_role "$MHAR_ROLE" \
  --out_dir "$MHAR_OUTPUT_DIR" \
  --wandb_project "MHAR Stuff" \
  --wandb_group "mhar-exp3-boundary-learnability-seed${MHAR_SEED}-branches" \
  --wandb_required \
  --run_name "mhar-exp3-seed${MHAR_SEED}-${MHAR_ROLE}" \
  "${MHAR_PARTITION_ARGS[@]}"
