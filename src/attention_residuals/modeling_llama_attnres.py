"""
Llama with Block Attention Residuals (AttnRes).
...
equivalent to standard Llama.  During training the bias and proj weights
...
# Fused Triton routing kernels (optional; enable via enable_fused_mhar() or
# ATTNRES_FUSED=1). delta_mh's mh_delta_attn_res dispatches to the fused
# self-contained path, which composes with FSDP and gradient checkpointing.
try:
    from . import mhar_triton as _mhar_fused
...
def enable_fused_mhar(enabled: bool = True):
    """Route delta_mh depth routing through fused Triton kernels (fwd+bwd)."""
...
class LlamaAttnResAttention(LlamaAttention):
...
class LlamaHeadwiseAttention(LlamaAttention):
    """LlamaAttention where each GQA head group reads its own routed input.
    ...
    LlamaAttention on the shared slice.
    """
...
class LlamaDepthAttention(LlamaAttention):
...
class LlamaAttnResConfig(LlamaConfig):
    """LlamaConfig extended with AttnRes hyper-parameters."""

    model_type = "llama_attnres"
...
_compiled_mh_delta_kernel = None
...
def enable_compile():
    ...
    global _compiled_mh_delta_kernel
    ...
    _compiled_mh_delta_kernel = torch.compile(
        _mh_delta_attn_res_kernel, dynamic=True)
...
def mh_block_attn_res(
    ...
    """
    if (
        _mhar_fused is not None
        and _mhar_fused.fused_mhar_enabled()
        and partial_block.is_cuda
        and blocks
    ):
        # Reuse the per-call fused delta kernel (composes with FSDP/grad_ckpt):
        # delta_route returns partial + mix(sources); with partial itself in the
        # source list, subtracting it back yields exactly mix(blocks+[partial]).
        return _mhar_fused.delta_route(
            blocks + [partial_block], partial_block, proj, norm, num_heads, None
        ) - partial_block
...
def hw_block_attn_res(
    ...
    LlamaHeadwiseAttention. Adds (G-1)*D params per call site vs block_attn_res.
    """
    V = torch.stack(blocks + [partial_block], dim=0)
...
def _mh_delta_attn_res_kernel(
    V: torch.Tensor,              # pre-stacked sources [N, B, T, D] (V[0]=null when used)
    partial_block: torch.Tensor,  # current residual stream  [B, T, D]
    query: torch.Tensor,          # (H, D/H) — per-head pseudo-queries
    norm: LlamaRMSNorm,
    num_heads: int,
) -> torch.Tensor:
    """Multi-head additive delta routing.

    Splits D into num_heads subspaces, each with its own softmax over depth
    sources (like _mh_block_attn_res_kernel), then ADDS the routed mix to the
    residual stream (additive/delta stream, like _delta_attn_res_kernel).
    No recency bias: with a zero-init query all content logits are 0, so the
    per-head softmax is uniform over sources at init.
    """
    K = norm(V)
    n, b, t, d = V.shape
    dh = d // num_heads
    logits = torch.einsum("h k, n b t h k -> n b t h", query, K.view(n, b, t, num_heads, dh))
    weights = logits.softmax(dim=0)
    out = torch.einsum("n b t h, n b t h k -> b t h k", weights, V.view(n, b, t, num_heads, dh))
    return partial_block + out.reshape(b, t, d)


def mh_delta_attn_res(
    deltas: list[torch.Tensor],   # previous block-aggregated deltas  [B, T, D] each
    partial_block: torch.Tensor,  # current residual stream  [B, T, D]
    proj: nn.Linear,              # learned query (d,) viewed as (H, d/H)
    norm: LlamaRMSNorm,
    num_heads: int,
    null_source: nn.Parameter | None = None,  # learnable null token (prepended as source 0)
) -> torch.Tensor:
    """Multi-head variant of delta_attn_res: attend over previous deltas with
    per-head depth routing and add the selected info to the residual stream.
    Mirrors delta_attn_res's null-source handling (identity source 0)."""
    if (
        _mhar_fused is not None
        and _mhar_fused.fused_mhar_enabled()
        and partial_block.is_cuda
        and (deltas or null_source is not None)
    ):
        return _mhar_fused.delta_route(
            deltas, partial_block, proj, norm, num_heads, null_source)
    ...
    V = torch.stack(sources, dim=0)
    query = proj.weight.view(num_heads, -1)
    kernel = _compiled_mh_delta_kernel or _mh_delta_attn_res_kernel
    return kernel(V, partial_block, query, norm, num_heads)
...
class LlamaAttnResDecoderLayer(GradientCheckpointingLayer):
    """
    Llama decoder layer with Block AttnRes via recency-biased depth attention.

    At init, a large recency bias makes block_attn_res return partial_block
    exactly, so the model is mathematically identical to standard Llama.
    ...
    def __init__(self, config: LlamaAttnResConfig, layer_idx: int):
        super().__init__()
        ...
        if self.attnres_mode in ("full_mh", "full_hw", "delta_mh", "block_mh") and config.hidden_size % self.attnres_num_heads != 0:
            raise ValueError(
        ...
        if self.attnres_mode in ("delta_v", "full_v", "full_split_shared_v"):
            self.self_attn = LlamaAttnResAttention(config=config, layer_idx=layer_idx)
        elif self.attnres_mode == "full_hw":
            self.self_attn = LlamaHeadwiseAttention(config=config, layer_idx=layer_idx)
        elif self.attnres_mode == "depth_attn":
            self.self_attn = LlamaDepthAttention(config=config, layer_idx=layer_idx)
        ...
        self.attention_type = "full_attention"  # Llama: no layer_types
        ...
        if self.gate_type == "sigmoid_scalar":
            ...
            self.attn_res_alpha = nn.Parameter(torch.zeros(1))  # 1D: FSDP rejects 0-dim scalar params
        ...
        self.use_null_source = getattr(config, "attnres_use_null_source", False) or self.attnres_mode == "delta_mh"
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.attnres_mode == "delta_mh":
            # ---- Delta-MH: delta_block additive stream with MULTI-HEAD routing ----
            # Same additive residual stream + block-aggregated delta sources as
            # delta_block, but per-head (subspace) depth routing and a zero-init
            # query. No recency bias. Reuses attn/mlp_res_proj/_norm and null sources.
            attn_null = self.attn_null_source if self.use_null_source else None
            mlp_null = self.mlp_null_source if self.use_null_source else None
            H = self.attnres_num_heads

            # Attention sublayer
            h_attn = mh_delta_attn_res(blocks, partial_block,
                                       self.attn_res_proj, self.attn_res_norm, H, attn_null)
            h = self._apply_gate(partial_block, h_attn, "attn")
            attn_out, _ = self.self_attn(
                hidden_states=self.input_layernorm(h),
                attention_mask=attention_mask,
            ...
            # MLP sublayer
            h_attn = mh_delta_attn_res(blocks, partial_block,
                                       self.mlp_res_proj, self.mlp_res_norm, H, mlp_null)
            h = self._apply_gate(partial_block, h_attn, "mlp")
            mlp_out = self.mlp(self.post_attention_layernorm(h))
            partial_block = partial_block + mlp_out
...
class LlamaAttnResModel(LlamaPreTrainedModel):
    """Llama backbone with Block AttnRes via recency-biased depth attention."""

    config_class = LlamaAttnResConfig

    def _init_weights(self, module):
        """Override to preserve AttnRes initialization."""
        super()._init_weights(module)
        if isinstance(module, LlamaAttnResDecoderLayer):
            gate_type = getattr(self.config, "attnres_gate_type", "bias")
            if gate_type == "sigmoid_scalar":
                if not getattr(module.attn_res_gate_logit, "_is_hf_initialized", False):
                    module.attn_res_gate_logit.data.fill_(-2.0)
                if not getattr(module.mlp_res_gate_logit, "_is_hf_initialized", False):
                ...
                # Guard: transformers 5.x re-applies _init_weights over bare scalar
                # Parameters after checkpoint loading on the direct-class
                # from_pretrained path (submodules like the routing Linears are
                # marked loaded and skipped; bare Parameters are not), silently
                # zeroing trained gates. Only initialize when the param has not
                # been populated from a checkpoint.
                if not getattr(module.attn_res_alpha, "_is_hf_initialized", False):
                    module.attn_res_alpha.data.fill_(0.0)
                if not getattr(module.mlp_res_alpha, "_is_hf_initialized", False):
                    module.mlp_res_alpha.data.fill_(0.0)
            if getattr(self.config, "attnres_mode", "block") == "delta_mh":
                # Zero-init the multi-head routing query (user spec: weight init = 0).
                # super()._init_weights already ran normal_ on these nn.Linear children
                # (post-order apply visits them before this layer), so re-zero here.
                nn.init.zeros_(module.attn_res_proj.weight)
                nn.init.zeros_(module.mlp_res_proj.weight)

    def __init__(self, config: LlamaAttnResConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaAttnResDecoderLayer(config, i) for i in range(config.num_hidden_layers)]
        )
        self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = LlamaRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
        self.has_sliding_layers = False  # Llama: no sliding layers
        ...
        self._needs_final_routing = self._attnres_mode in (
            "block", "block_v", "block_mh", "full", "full_mh", "full_hw", "full_split",
            "delta_centered_reset",
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        is_delta_block = attnres_mode in ("delta_block", "delta_block_v", "delta_mh")
        ...
        elif attnres_mode in ("full_mh", "full_hw", "block_mh"):
            # block_mh resets partial at block starts, so the final state must be
            # routed over all blocks (Qwen3's block_mh skips this — a bug we fix here).
...
class LlamaAttnResForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    """Llama causal LM with Block AttnRes residuals."""
    ...
    def __init__(self, config: LlamaAttnResConfig):
        super().__init__(config)
        self.model = LlamaAttnResModel(config)
        self.vocab_size = config.vocab_size
