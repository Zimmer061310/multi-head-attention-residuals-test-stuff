...
MHAR_DATA_DIR="$MHAR_DATA_DIR" "$MHAR_PYTHON_BIN" \
  "$MHAR_REPO_DIR/download_stage_b_data.py" --output-dir "$MHAR_DATA_DIR"

MHAR_DATA_DIR="$MHAR_DATA_DIR" "$MHAR_PYTHON_BIN" \
  "$MHAR_REPO_DIR/preflight_stage_b_server.py"

cd "$MHAR_REPO_DIR"
MHAR_RUN_MODEL_INTEGRATION=1 "$MHAR_PYTHON_BIN" -m unittest -v \
  test_experiment2_stage_b_screening.py test_train_resume.py
