...
if [[ ! -f "$MHAR_FIXED_ARTIFACT" ]]; then
  "$MHAR_PYTHON_BIN" experiment1_partition_compatibility.py materialize \
...
"$MHAR_PYTHON_BIN" experiment1_partition_compatibility.py evaluate \
  "${COMMON_EVAL_ARGS[@]}" \
...
"$MHAR_PYTHON_BIN" experiment1_partition_compatibility.py analyze \
